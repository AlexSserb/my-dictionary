...
from app.schemes.schemes import (
    WordSchema,
...
class TestWordModels(BaseTestCase):
    ...
    def test_get_words_for_dictionary(self):
        ...
        self.assertEqual(
            Word.get_for_dictionary(self.dict1),
            [
                word1,
            ],
        )
    ...
    def test_get_words_and_translations_for_dictionary(self):
        ...
        words_and_translations1 = (
            WordTranslation.get_words_and_translations_for_dictionary(self.dict1)
        )
        words_and_translations2 = (
            WordTranslation.get_words_and_translations_for_dictionary(self.dict2)
        )

        self.assertEqual(
            set(words_and_translations1),
            set((("word1", "translation1"), ("word2", "translation2"))),
        )
    ...
    def test_create_word_and_translations(self):
        ...
        with Session(db.engine) as session:
            ...
            self.assertEqual(
                saved_word.dictionary,
                session.get(Dictionary, data_to_create.dictionary_id),
            )
            self.assertEqual(
                set(saved_word.translations),
                set(
                    (
    ...
    def test_update_word_and_translations(self):
        ...
        with Session(db.engine) as session:
            ...
            self.assertEqual(
                saved_word.dictionary,
                session.get(Dictionary, data_to_update.dictionary_id),
            )
            self.assertEqual(
                set(saved_word.translations),
                set(
    ...
    def test_apply_training_results(self):
        ...
        training_results = ListOfTrainingResultSchema(
            training_results=[
