...
class TestAccountsAPI(BaseTestCaseWithoutDB):
    ...
    def test_success_registration(self, mock_get_by_identity, mock_register_user):
        ...
        response = self.client.post('/accounts/register', json={
            'username': username,
            'password': password
        })
    ...
    @patch('app.database.User.check_password')
    @patch('app.database.User.get_by_identity')
    def test_failed_login_ivalid_password(self, mock_get_by_identity, mock_check_password):
        username, password = 'UserName', 'mysecretpassword'
        user = User(username=username)
        user.set_password(password)
        mock_get_by_identity.return_value = user
    ...
    def test_refresh_token(self):
        ...
        refresh_token = create_refresh_token(identity=username, additional_claims=payload)
        response = self.client.post('/accounts/refresh', headers={ 'Authorization': f'Bearer {refresh_token}' })
