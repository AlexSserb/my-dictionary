...
class TestWordsAPI(BaseTestCaseWithoutDB):
    ...
    def setUp(self):
        ...
        token = create_access_token(
            self.user.username, additional_claims={"username": self.user.username}
        )
    ...
    def _set_test_data_get_words_for_dict(self):
        ...
        """
        self.word1 = Word(
            id=uuid4(), word="word1", dictionary_id=self.dict1.id, progress=5
        )
        self.word2 = Word(
            id=uuid4(), word="word2", dictionary_id=self.dict2.id, progress=10
        )

        self.word_translation1 = WordTranslation(
            id=uuid4(), translation="translation1", word_id=self.word1.id
        )
        self.word_translation2 = WordTranslation(
            id=uuid4(), translation="translation2", word_id=self.word1.id
        )
        self.word_translation3 = WordTranslation(
            id=uuid4(), translation="translation3", word_id=self.word2.id
        )

        self.word1.translations, self.word2.translations = [
            self.word_translation1,
            self.word_translation2,
        ], [
            self.word_translation3,
        ]
    ...
    def test_get_words_for_dict(self, mock_get_for_dictionary, mock_get_dict_or_404):
        ...
        self.assertEqual(
            set((t["translation"] for t in words[0]["translations"])),
            set(
        ...
        self.assertEqual(
            words[1]["translations"][0]["translation"],
            self.word_translation3.translation,
        )
