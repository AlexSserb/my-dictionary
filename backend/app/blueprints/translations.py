...
def translate_google(body: TranslateSchema):
    ...
    translation_result = translator.translate(body.text, src=body.language_from.code, dest=body.language_to.code)

    result_set_of_translations = []

    for group in translation_result.extra_data['possible-translations']:
        for translation in group[2]:
            result_set_of_translations.append({ 'id': uuid4(), 'translation': translation[0] })
    
    return jsonify(TranslationGoogleResult(
        translations=result_set_of_translations
